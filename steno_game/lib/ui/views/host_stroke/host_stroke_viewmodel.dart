...
HostStrokeViewModel(this.game);
  ...
  void readyGame() async {
    ...
    response.fold((l) => showBottomSheet(l.message), (r) {
      game = MultiplayerStroke(id: game.id, gameMaster: game.gameMaster, status: 1);
  ...
  Future<void> addStrokeQuiz() async {
    setBusy(true);
    final addStrokeResponse = await _strokeRepo.addStroke(painterKey, textController.text, 1,null);
    addStrokeResponse.fold((l) {
      showBottomSheet(l.message);
      print(l.message);
    }, (stroke) async{
      final response = await _multiStrokeRepo.addQuestion(game.id, stroke.strokeImage, "stroke");
  ...
  Future<void> addTextQuiz() async {
    setBusy(true);
    final response = await _multiStrokeRepo.addQuestion(game.id, textController.text, "text");
    response.fold((l) => showBottomSheet(l.message), (quiz){
      questions.add(quiz);
      cancelAdd();
      rebuildUi();
