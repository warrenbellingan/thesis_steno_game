...
class AddEditQuizViewModel extends BaseViewModel {
  ...
  void addQuiz() async {
    ...
    if(quizzes == null) {
      ...
      final response = await _quizRepo.updateQuizzes(quizzes!.id, titleTextController.text);
      response.fold((l) => showBottomSheet(l.message), (r) async {
        quizzes = r;
        await getQuiz();
        isEditing = true;
  ...
  List<String> getChoices() {
    return [correctTextController.text, wrong1TextController.text, wrong3TextController.text, wrong2TextController.text];
  }
